...
public class principalExtra {
    ...
    public static void main(String[] args) {
        ...
        // Generamos el tablero de forma aleatoria (Opcional +3 puntos)
        int[][] tableroDescubierto = generarTableroAleatorio(N);
        ...
        while (intentosJugador > 0 && !juegoTerminado) {
            ...
            if (tableroDescubierto[cordX1][cordY1] == tableroDescubierto[cordX2][cordY2]) {
                ...
                // No se restan intentos. Las casillas ya están a 'true', se quedan así.
    ...
    /**
     * Genera una matriz NxN con parejas de números distribuidas aleatoriamente.
     * Utiliza arrays estáticos estrictamente para realizar el reparto aleatorio.
     */
    public static int[][] generarTableroAleatorio(int N) {
        int totalCasillas = N * N;
        int[] paresLineales = new int[totalCasillas];

        // 1. Llenamos un array unidimensional con las parejas (ej: 1,1, 2,2, 3,3...)
        int numeroPareja = 1;
        for (int i = 0; i < totalCasillas; i += 2) {
            paresLineales[i] = numeroPareja;
            paresLineales[i + 1] = numeroPareja;
            numeroPareja++;
        }

        // 2. Desordenamos el array (Algoritmo Fisher-Yates usando solo arrays)
        Random rand = new Random();
        for (int i = paresLineales.length - 1; i > 0; i--) {
            int j = rand.nextInt(i + 1);
            // Intercambio
            int temp = paresLineales[i];
            paresLineales[i] = paresLineales[j];
            paresLineales[j] = temp;
        }

        // 3. Volcamos el array desordenado en la matriz bidimensional
        int[][] tableroNuevo = new int[N][N];
        int indice = 0;
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; j++) {
                tableroNuevo[i][j] = paresLineales[indice];
                indice++;
            }
        }
